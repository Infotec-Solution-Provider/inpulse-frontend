...
class AiClient extends api_client_1.default {
    ...
    async patchSupervisorSession(sessionId, data, token) {
        const { data: res } = await this.ax.patch(`/api/ai/supervisor-chat/sessions/${sessionId}`, data, { headers: this.authHeader(token) });
        return res.data;
    }
    ...
    async decideSupervisorAction(sessionId, actionId, data, token) {
        const { data: res } = await this.ax.post(`/api/ai/supervisor-chat/sessions/${sessionId}/actions/${actionId}/decision`, data, { headers: this.authHeader(token) });
        return res.data;
    }
