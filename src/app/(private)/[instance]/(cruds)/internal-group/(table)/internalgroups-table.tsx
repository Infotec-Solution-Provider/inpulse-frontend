...
import InternalGroupsTableRow from "./internalgroups-row";
import { StyledTableCell, StyledTableRow } from "./styles-table";

export default function InternalGroupsTable() {
  const { internalgroups, loading, sortedInternalGroups, order, orderBy, handleSort, openInternalGroupModal } = useContext(InternalGroupsContext);
  const [page, setPage] = useState<number>(0);
  const [rowsPerPage, setRowsPerPage] = useState(10);

  useEffect(() => setPage(0), [internalgroups]);
  ...
  const rows = useMemo(() => {
    if (!loading && sortedInternalGroups.length > 0) {
      return (
        <TableBody>
          {sortedInternalGroups.slice(page * rowsPerPage, (page + 1) * rowsPerPage).map((internalgroup, index) => (
            <InternalGroupsTableRow
              key={`${internalgroup.NOME}_${internalgroup.CODIGO}`}
  ...
  }, [sortedInternalGroups, page, rowsPerPage, loading]);
  ...
  return (
    <div className="relative flex flex-col h-[calc(100vh-100px)]">
      ...
      <div className="flex sticky bottom-0 self-center pt-4 pb-2">
        <Button onClick={() => openInternalGroupModal()} variant="outlined">
          Cadastrar usuário
        </Button>
        <TablePagination
          component="div"
          count={internalgroups.length}
          page={page}
