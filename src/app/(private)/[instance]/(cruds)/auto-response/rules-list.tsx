...
export default function RulesList() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto w-full h-full flex flex-col min-h-0">
      <header className="flex flex-col sm:flex-row justify-between sm:items-center mb-4 sm:mb-6 sticky top-0 z-10 bg-slate-50/80 dark:bg-gray-900/80 backdrop-blur supports-[backdrop-filter]:bg-slate-50/60 dark:supports-[backdrop-filter]:bg-gray-900/60 py-3">
        <div>
      ...
      {rules.length > 0 ? (
        <div
          className="flex-1 overflow-y-auto  scrollbar-whatsapp min-h-0"
          role="region"
          aria-label="Lista de regras de resposta automática"
        >
          <main className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-8 pr-1">
            {rules.map((rule) => (
        ...
        <div className="flex-1 overflow-y-auto  scrollbar-whatsapp min-h-0">
