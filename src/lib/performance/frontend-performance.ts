...
interface PersistedTelemetrySession {
  sessionId: string;
  startedAt: string;
...
export interface FrontendErrorContext {
  source?: "window_error" | "unhandled_rejection" | "api" | "file_send";
  phase?: string;
}
...
const SESSION_STORAGE_PREFIX = "@inpulse/frontend-performance/session/v2";
...
function telemetryErrorCategory(value: unknown, sanitizedMessage: string): string {
  const candidate = value as { code?: unknown; name?: unknown } | null;
  const code = typeof candidate?.code === "string" ? candidate.code.toUpperCase() : "";
  const name = typeof candidate?.name === "string" ? candidate.name : "";
  const normalizedMessage = sanitizedMessage.toLowerCase();
  if (name === "AbortError" || code === "ERR_CANCELED" || /abort|cancel/.test(normalizedMessage)) {
    return "aborted";
  }
  if (
    code === "ECONNABORTED" ||
  ...
  if (
    /chunkloaderror|loading chunk|failed to fetch dynamically imported module/.test(
      normalizedMessage,
    )
  ) {
    return "chunk_load";
  }
  if (name === "QuotaExceededError" || /quota exceeded/.test(normalizedMessage)) return "quota";
  if (
    code === "ERR_NETWORK" ||
    name === "NetworkError" ||
    /network|failed to fetch/.test(normalizedMessage)
  ) {
    return "network";
  }
...
export function createTelemetryErrorTags(
  ...
  return {
    ...
    errorCategory: telemetryErrorCategory(value, sanitizedMessage),
    ...(context.source ? { errorSource: context.source } : {}),
...
function isDeviceMetadata(value: unknown): value is DeviceMetadata {
  ...
  const device = value as Partial<DeviceMetadata>;
  return (
    typeof device.browser === "string" &&
    (device.hardwareConcurrency === null || typeof device.hardwareConcurrency === "number") &&
    (device.deviceMemoryGb === null || typeof device.deviceMemoryGb === "number") &&
    (device.effectiveType === null || typeof device.effectiveType === "string") &&
    typeof device.viewportWidth === "number" &&
    typeof device.viewportHeight === "number"
  );
}

function createTelemetrySession(): PersistedTelemetrySession {
  const device = getDeviceMetadata();
  return {
    sessionId: crypto.randomUUID(),
    startedAt: new Date().toISOString(),
    device,
    detailed: shouldCollectDetailedMetrics(device, randomRatio()),
  };
}

export function isReusableTelemetrySession(
  value: unknown,
  now = Date.now(),
): value is PersistedTelemetrySession {
  if (!value || typeof value !== "object") return false;
  const session = value as Partial<PersistedTelemetrySession>;
  ...
  return (
    ...
    now - startedAt >= 0 &&
    now - startedAt < SESSION_ROTATION_INTERVAL_MS &&
    isDeviceMetadata(session.device) &&
    typeof session.detailed === "boolean"
  );
}

function getOrCreateTelemetrySession(buildId: string): PersistedTelemetrySession {
  const storageKey = `${SESSION_STORAGE_PREFIX}/${buildId}`;
  try {
    const raw = window.sessionStorage.getItem(storageKey);
    if (raw) {
      const parsed = JSON.parse(raw) as unknown;
      if (isReusableTelemetrySession(parsed)) return parsed;
    }
    const created = createTelemetrySession();
...
function replaceTelemetrySession(buildId: string): PersistedTelemetrySession {
  const created = createTelemetrySession();
  try {
    window.sessionStorage.setItem(`${SESSION_STORAGE_PREFIX}/${buildId}`, JSON.stringify(created));
  } catch {
...
class FrontendPerformanceCollector {
  ...
  private documentMetricsCaptured = false;
  private documentObserversInstalled = false;
  ...
  public start(options: { token: string; endpoint: string; buildId: string; route: string }) {
    ...
    this.route = this.normalizeRoute(options.route);
    const session = getOrCreateTelemetrySession(this.buildId);
    this.sessionId = session.sessionId;
    this.startedAt = session.startedAt;
    this.sessionStartedWallTime = new Date(session.startedAt).getTime();
    this.device = session.device;
    this.detailed = session.detailed;
    ...
    const replayBufferedEntries = !this.documentObserversInstalled;
    this.installObservers(replayBufferedEntries);
    this.documentObserversInstalled = true;
    this.installGlobalListeners();
    this.installDefaultAxiosInstrumentation();
    if (this.isDocumentMetricsOwner(this.sessionId) && !this.documentMetricsCaptured) {
      this.captureNavigation();
      this.documentMetricsCaptured = true;
    }
  ...
  private async send(
    ...
    try {
      ...
      if (allowRetry && (detached || (this.enabled && envelope.generation === this.generation))) {
  ...
  private installObservers(replayBufferedEntries: boolean) {
    ...
    if (supported.includes("longtask")) {
      ...
      observer.observe({ type: "longtask", buffered: replayBufferedEntries });
      this.observers.push(observer);
    }

    if (
      this.isDocumentMetricsOwner(this.sessionId) &&
      supported.includes("paint") &&
      !this.reportedWebVitals.has("web_vital.fcp")
    ) {
      const observer = new PerformanceObserver((list) => {
        if (generation !== this.generation) return;
      ...
      observer.observe({ type: "paint", buffered: replayBufferedEntries });
      this.observers.push(observer);
    }
  ...
  private installGlobalListeners() {
    ...
    const onError = (event: ErrorEvent) =>
      this.captureError(event.error ?? event.message, { source: "window_error" });
    const onRejection = (event: PromiseRejectionEvent) =>
      this.captureError(event.reason, { source: "unhandled_rejection" });
  ...
  public captureError(value: unknown, context: FrontendErrorContext = {}) {
    this.record({
      name: "error.count",
      value: 1,
      unit: "count",
      tags: createTelemetryErrorTags(value, context),
    });
  }
  ...
  private aggregateResourceTiming(endpoint: string, entry: PerformanceResourceTiming) {
    ...
    if (
      !this.resourceAggregates.has(key) &&
      this.resourceAggregates.size >= MAX_RESOURCE_AGGREGATES
    ) {
      key = `${this.route}\u0000/api/browser-resource/other\u0000other`;
    }
    const [
      route = "/unknown",
      aggregateEndpoint = "/api/browser-resource/other",
      aggregateInitiator = "other",
    ] = key.split("\u0000");
  ...
  private rotateSession() {
    ...
    replaceTelemetrySession(this.buildId);
    this.stop();
    this.start(options);
  }
...
export function recordFrontendError(value: unknown, context: FrontendErrorContext = {}) {
  frontendPerformanceCollector.captureError(value, context);
}
...
export async function measuredFrontendFetch(
  ...
  try {
    ...
    if (frontendPerformanceCollector.getSessionId() === sessionId) {
      ...
      recordFrontendError(error, { source: "api" });
    }
    throw error;
  }
}
